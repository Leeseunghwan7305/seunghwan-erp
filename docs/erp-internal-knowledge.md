...
> **기준일:** 2026-09-16
> **범위:** 회사·조직 정보 / 업무 규정·프로세스(SOP) / 용어집·데이터 사전 / 현재 데이터 스냅샷
...
## 4. 현재 데이터 스냅샷 (기준일 2026-09-16)

> 아래 수치는 기준일 시점의 값이다. 최신 값은 ERP 조회 도구를 사용할 것.
...
| B-002 | 위생장갑 100매 | EA | 2,000 | 3,500 | 30 | 12 | **안전재고 미달(부족 18)** |
...
### 4.4 주문·전표
| 주문번호 | 유형 | 거래처 | 일자 | 상태 | 합계 | 전표 | 결제 |
|----------|------|--------|------|------|------|------|------|
| 1 | 발주 | 대성물류 | 2026-09-16 | 확정 | 120,000 | 매입 | 미결제 |

- 주문 #1 상세: 탄산수 350ml × 20 BOX @ 6,000 = 120,000원
...
*문서 생성: 2026-09-16 · 출처: seunghwan-erp 로컬 DB(erp) · 재생성: `docs/build_knowledge_pdf.py`*
